...
export default (targetValue: { [x: string]: { responses: {} } } | null, { wellUnderstood }: any, context: { path: any }) => {
  const result = []
  ...
  for (const verb of Object.keys(targetValue)) {
    ...
    for (const code of Object.keys(responses)) {
      ...
      const allowedVerbs = wellUnderstood[code].map((verb) =>
        verb.toUpperCase()
      )
      const upperCaseVerb = verb.toUpperCase()
      if (
        !allowedVerbs.includes('ALL') &&
        !allowedVerbs.includes(upperCaseVerb)
      ) {
        result.push({
